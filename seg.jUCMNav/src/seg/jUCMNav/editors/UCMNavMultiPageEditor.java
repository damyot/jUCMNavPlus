...
/**
 * This class is the central location for our editor. Its main responsibilities
 * are as follows.
 * 
 * 1) The editor associated with .jucm files. See
 * seg.jUCMNav.editors.resourceManagement.MultiPageFileManager.
 * 
 * 2) An editor itself, has an outline when no maps are available. Its model is
 * the root URNspec. See UcmOutlinePage.
 * 
 * 3) A container for UcmEditors, which edit only one map. Some aspects are
 * delegated to the children but some are handled here.
 * 
 * 3.1) For tab management, see MultiPageTabManager.
 * 
 * 3.2) For command stack management (delegated to children), see
 * MultiPageCommandStackListener, DelegatingCommandStack.
 * 
...
public class UCMNavMultiPageEditor extends MultiPageEditorPart implements Adapter, INavigationLocationProvider,
		IGotoMarker, IPropertySourceProvider, ITabbedPropertySheetPageContributor {
	private final class ActionRegistrySelectionListener implements ISelectionListener {
		...
		public void selectionChanged(IWorkbenchPart part, ISelection selection) {
			...
			AlignStateSourceProvider alignStateSourceProvider = (AlignStateSourceProvider) service
					.getSourceProvider(AlignStateSourceProvider.SELECTION_STATE);
	...
	/**
	 * keeps track of different types of actions, that sometimes need to be
	 * accessed later
	 */
	...
	/**
	 * this boolean is used to persist the fact that we might have performed
	 * stack actions elsewhere than in the individual editor stacks.
	 */
	...
	/**
	 * to simplify the code in this class, delegate tab management to
	 * MultiPageTabManager
	 */
	...
	public void dispose() {
		...
		getSite().getWorkbenchWindow().getSelectionService()
				.removeSelectionListener(getMultiPageTabManager().getSelectionListener());
	...
	public void doSave(IProgressMonitor monitor) {
		getFileManager().setSaveType(getFileManager().SAVE);
	...
	/**
	 * Returns the action registry manager. We delegate action registry
	 ...
	 * 
	 * To note: If we don't have any maps in the file (actually, opened tabs),
	 * we will show a default outline page.
	 * 
	 * If we don't have an adapter defined here and there are opened tabs, we
	 * will delegate to the active editor instead of our superclass.
	 * 
	 */
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		try {
			...
			// doing this here (even though done in super.init() because we want
			// the following invocation to be able to popup an error message
			// should anything
			// fail.
			setSite(site);
			...
			if (!(input instanceof IFileEditorInput)) {
				...
				// we don't want to open it because our scenario warnings are
				// associated to the IFile...
				MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						Messages.getString("UCMNavMultiPageEditor.Error"), //$NON-NLS-1$
						Messages.getString("UCMNavMultiPageEditor.CannotUseFileOpen")); //$NON-NLS-1$
				try {
		...
		getSite().getWorkbenchWindow().getSelectionService()
				.addSelectionListener(getMultiPageTabManager().getSelectionListener());

		// Creates actions and registers them to the ActionRegistry. Because the
		// action registry is shared, we must created them here.
		...
		try {
			...
			if (value != null && value.equals("true") && getModel().getUrndef().getConcerns().size() > 0) {
				...
				// when loading and opening the editor of file, check if the
				// name of concern is matched with the name of file
				IFile file = ((IFileEditorInput) input).getFile();
				...
				if (tempConcern != null && (tempConcern.get(0)) != null) {
					Concern formal_Concern = (Concern) tempConcern.get(0);
					if (!formal_Concern.getName().equals(potent_ConcernName)) {

						formal_Concern.setName(potent_ConcernName);
						for (Iterator<IntentionalElement> itr = ((URNspec) getModel()).getGrlspec().getIntElements()
							...
							if (intElement instanceof Feature && valueRootFea != null
									&& valueRootFea.equalsIgnoreCase("root feature")) {
								intElement.setName(potent_ConcernName);
							}
							System.out.println("The type of intelement is task "
									+ (intElement.getType().getValue() == IntentionalElementType.TASK));
							System.out.println("If the intelement is an instance of Feature.class"
									+ (intElement instanceof Feature));
						}
					}
				}
				getFileManager().setSaveType(getFileManager().INIT);
				getFileManager().doSave(new NullProgressMonitor());
				// update the corresponding .core file to the .jucm file,check
				// if the .core with specific name has been created

				IWorkspaceRoot root = ResourcesPlugin.getWorkspace().getRoot();
				String corefilename = File.separator + file.getProject().toString().substring(2) + File.separator
						+ fileName + "." + "core";
				IResource resource = root.findMember(new Path(corefilename));

				String coreFilefullpath = file.getLocation().removeFileExtension().toString() + "." + "core";
				System.out.println(new File(coreFilefullpath).getAbsolutePath());
	...
	/**
	 * Is this editor dirty? Through the MultiPageCommandStackListener and the
	 * DelegatingCommandStack behaviour, this means that either one of the
	 * editor stacks is dirty or the URNspec stack has been modified.
	 */
	...
	public void setModel(URNspec model) {
		...
		// we must register ourselves to be able to change the tabs when the
		// names change.
	...
	/**
	 * If the marker has an EObject attribute, the value is its
	 * {@link URNmodelElement} ID. We try to find it in the {@link URNspec} and
	 * select it in the UI.
	 * 
	 * Otherwise, we look at its resolutions and if a code editor must be opened
	 * to fix it (via a {@link OpenEditorQuickFix}), we open the code editor.
	 */
		...
		try {
			...
			if (o != null) {
				...
				if (resolutions.length > 0) {
					// we don't want to automatically run any other quick fix
					// than this one.
