...
import seg.UCMScenarioViewer.parser.EMFParserInitializer;
import seg.UCMScenarioViewer.parser.MSCParserInitializer;
import seg.UCMScenarioViewer.parser.XMLParserInitializer;
...
public class ImportWizardFirstPage extends WizardPage {
	...
	/*
	 * TODO - move the parsing logic outside the validation method, this is confusing!
	 */
		...
		try {
			...
			if (file.isFile()) {
				...
				if (fileName.toLowerCase().endsWith(Helper.EXTENSION_XML)) {
					...
					setErrorMessage(null);
				} else if (fileName.toLowerCase().endsWith(Helper.EXTENSION_JUCMSCENARIOS)) {
					model = EMFParserInitializer.parseMscModel(file);
				} else {
			...
			e.printStackTrace();
			setErrorMessage(MessageFormat.format(Helper.WIZARD_P1_CANT_PARSE_ERROR, new String[] { file.getName() }));
		}
