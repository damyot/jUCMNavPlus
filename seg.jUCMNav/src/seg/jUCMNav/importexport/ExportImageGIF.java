    ...
    public void export(final IFigure pane, final FileOutputStream fos) {
        ...
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                ...
                // Scale-nudge workaround for issue #4 (see CopyAction and
                // ExportImage parent for the matching fixes). At zoom
                // EXACTLY 1.0 the live ScalableFreeformLayeredPane skips
                // the ScaledGraphics wrap and antialiased polygon fills
                // (stubs, endpoints, forks/joins, direction arrows)
                // silently drop on the off-screen GC. Nudging the scale
                // forces the working code path.
                //
                // Earlier revision used `instanceof ScalableLayeredPane`
                // which never matched because the jUCMNav editor uses
                // ScalableFreeformLayeredPane (a SIBLING class, not a
                // subclass) -- so the nudge never fired and the GIFs
                // generated by the HTML report were still missing every
                // polygon-based symbol. Use IScalablePane, the common
                // interface implemented by both panes. Allocate the image
                // AFTER the nudge so its dimensions match what we paint.
                final IScalablePane scalablePane = (pane instanceof IScalablePane)
                        ? (IScalablePane) pane : null;
                ...
                try {
                    if (needsNudge) {
                        scalablePane.setScale(originalScale + 0.01);
                    }
